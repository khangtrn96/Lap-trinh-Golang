...
//Hàm creatPrintFunction là ứng dụng của Closure
func creatPrintFunction(custom string) xapxepketqua {
	a := func(x string) {
		fmt.Println(x + custom + x)
	}
	return a
}
func main() {
	yourobject := Salutation{"Toán", "Văn", "Lý", "Hoá"}
	showresult(yourobject, creatPrintFunction("!!!"))
}
